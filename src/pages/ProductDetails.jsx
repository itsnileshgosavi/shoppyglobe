...
import { useGetProductByIdQuery } from "../utils/redux/apiSlice";
...
function ProductDetails() {
  ...
  const { data, isLoading: loading, error } = useGetProductByIdQuery(id); //fetching data from api using custom hook
    ...
    <div className="flex flex-col">
          ...
          <div className="flex justify-start flex-col md:flex-row items-center my-2">
            ...
            {cart.products.some((p) => p.id === product.id) ? (
              <>
                ...
                />
                <Link to="/cart">
                  <button className="bg-green-500 my-5 hover:bg-green-700 rounded-md shadow-lg text-white font-bold px-5 mx-5 py-2 active:scale-95">
                    Go to cart{" "}
                  </button>
                </Link>
              </>
      ...
      <div className="flex flex-col justify-center mx-auto my-5">
        ...
        <div className="grid grid-cols-2 my-5 capitalize">
          <div>
            {product.brand && (
              <p className="text-sm md:text-xl font-medium">Brand:</p>
            )}
